...
TEST_CASE("agent.service_registration", "[consul][agent][services]")
{
    ...
    SECTION("no check simple")
    {
        ...
        CHECK(s.port == 1357);
        CHECK(s.tags == ppconsul::agent::Tags({ "tag1" }));
    }
    ...
    {
        agent.registerService({ "service1" }, Non_Existing_Script_Name, std::chrono::minutes(1));
...
TEST_CASE("agent.service_deregistration_with_script", "[consul][agent][services]")
{
    ...
    agent.registerService({ "service1" }, std::chrono::seconds(10));
...
TEST_CASE("agent.service_registration_special_chars", "[consul][agent][service][special chars]")
{
    ...
    SECTION("ttl")
    {
        agent.registerService({ spec_name }, std::chrono::minutes(5));
    ...
    SECTION("script")
    {
        agent.registerService({ spec_name }, Non_Existing_Script_Name, std::chrono::minutes(1));
