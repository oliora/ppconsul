...
TEST_CASE("agent.check_deregistration", "[consul][agent][checks]")
{
    ...
    agent.registerCheck({ "check1", ppconsul::TtlCheckConfig{std::chrono::seconds(12)} });
...
TEST_CASE("agent.check_registration", "[consul][agent][checks]")
{
    ...
    SECTION("ttl")
    {
        agent.registerCheck({ "check1", ppconsul::TtlCheckConfig{std::chrono::seconds(180)} });
    ...
    {
        agent.registerCheck({ "check1", ppconsul::TtlCheckConfig{std::chrono::seconds(180)}, "some notes" });
    ...
    SECTION("ttl with id")
    {
        agent.registerCheck({ "check1", ppconsul::TtlCheckConfig{std::chrono::seconds(180)}, "other notes", Unique_Id });
    ...
    SECTION("script")
    {
        agent.registerCheck({ "check1", ppconsul::ScriptCheckConfig{Non_Existing_Script_Name, std::chrono::seconds(100)} });
    ...
    {
        agent.registerCheck({ "check1", ppconsul::ScriptCheckConfig{Non_Existing_Script_Name, std::chrono::seconds(100)}, "the notes" });
        sleep(0.5); // To get updated state and output
    ...
    {
        agent.registerCheck({ "check1", ppconsul::ScriptCheckConfig{Non_Existing_Script_Name, std::chrono::seconds(1)}, "the notes", Unique_Id });
        sleep(0.5); // To get updated state and output
...
TEST_CASE("agent.check_registration_special_chars", "[consul][agent][checks][special chars]")
{
    ...
    SECTION("ttl")
    {
        agent.registerCheck({ spec_name, ppconsul::TtlCheckConfig{std::chrono::seconds(100)}, "note\n1\n2\n3\nsummary: bla bla" });
    ...
    SECTION("script")
    {
        agent.registerCheck({ spec_name,  ppconsul::ScriptCheckConfig{Non_Existing_Script_Name, std::chrono::seconds(100)} });
        sleep(0.5); // To get updated state and output
...
TEST_CASE("agent.check_update", "[consul][agent][checks][health]")
{
    ...
    agent.registerCheck({ "check1", ppconsul::TtlCheckConfig{std::chrono::minutes(5)}, "the check" });
...
TEST_CASE("agent.check_update_incorrect", "[consul][agent][checks][health]")
{
    ...
    SECTION("script")
    {
        agent.registerCheck({ "check1" , ppconsul::ScriptCheckConfig{Non_Existing_Script_Name, std::chrono::minutes(5)} });

        CHECK_THROWS_AS(agent.pass("check1"), ppconsul::Error);
    }
    SECTION("http")
    {
        agent.registerCheck({ "check1" , ppconsul::HttpCheckConfig{"invalid.", std::chrono::minutes(5)} });

        CHECK_THROWS_AS(agent.pass("check1"), ppconsul::Error);
    }
    SECTION("tcp")
    {
        agent.registerCheck({ "check1" , ppconsul::TcpCheckConfig{"127.0.0.1:0", std::chrono::minutes(5)} });

        CHECK_THROWS_AS(agent.pass("check1"), ppconsul::Error);
    }
    SECTION("docker")
    {
        agent.registerCheck({ "check1" , ppconsul::DockerCheckConfig{"example.com-docker-unexisting", Non_Existing_Script_Name, std::chrono::minutes(5)} });
...
TEST_CASE("agent.check_expired", "[consul][agent][checks][health]")
{
    ...
    agent.registerCheck({ "check1", ppconsul::TtlCheckConfig(std::chrono::seconds(1))});
    REQUIRE(agent.checks().count("check1"));
